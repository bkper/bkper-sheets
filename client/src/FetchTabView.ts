namespace FetchTabView {
    ...
    export function shouldFetchValues(): boolean {
        return $('input[name=fetch-option-radio]:checked', view.fetchOptionRadioGroup).val() ===
            'VALUES'
            ? true
            : false;
    }
                ...
                source: function (request, response) {
                    ...
                    if (getFetchType() === 'accounts') {
                        ...
                        filteredQueries = queries.filter(
                            (q: any) => q.category === QUERY_CATEGORY_GROUP_
                        );
                    } else if (getFetchType() !== 'transactions') {
                        filteredQueries = queries.filter(
                            (q: any) => q.category !== QUERY_CATEGORY_STATUS_
                        );
                    }
