namespace FetchTabActivity {
    ...
    export function loadQueries(ledgerId) {
        ...
        if (ledgerId != null && ledgerId.trim() != '') {
            ...
            if (cachedQueries != null) {
                ...
                if (!DEV_MODE) {
                    google.script.run
                        .withSuccessHandler(queriesLoaded)
                        .withFailureHandler(SidebarView.showError)
                        .loadQueries(ledgerId);
                }
            }
            if (DEV_MODE) {
                FetchTabView.setQueries(cachedQueries);
    ...
    export function fetchQuery() {
        ...
        google.script.run
            .withSuccessHandler(fetched)
            .withFailureHandler(FetchTabView.showQueryError)
            .fetchQuery(form, fetchValues);
    }
