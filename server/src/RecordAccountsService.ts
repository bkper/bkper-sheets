namespace RecordAccountsService {
    ...
    export function batchCreateAccounts(
        ...
        let backgrounds: any[][] = initilizeMatrix(
            new Array(values.length),
            header.getColumns().length
        );
        ...
        if (bookIdHeaderColumn) {
            ...
            for (let i = startAt; i < values.length; i++) {
                ...
                if (bookId != '') {
                    ...
                    if (batch == null) {
                        batch = new RecordAccountBatch(bookIdValidation.booksById[bookId]);
            ...
            for (const key in accountsBatch) {
                ...
                if (highlight) {
                    backgrounds = updateBackgroundsArray(
                        backgrounds,
                        batch,
                        header.getColumns().length
                    );
    ...
    function updateBackgroundsArray(
        ...
        batch: RecordAccountBatch,
        headerLength: number
    ): any[][] {
        const accountTypesMap = batch.getAccountTypesMap();
        for (const key of Object.keys(accountTypesMap)) {
            backgroundsArray[+key] = fill(
                new Array(headerLength),
                getTypeColor(accountTypesMap[key])
            );
        }
        return backgroundsArray;
    }

    function arrayToBatch_(
        row: any[],
        batch: RecordAccountBatch,
        ...
        if (header.isValid()) {
            for (const column of header.getColumns()) {
                ...
                if (column.isName()) {
                    ...
                    accountType = isValidType(value)
                        ? (value as Bkper.AccountType)
                        : BkperApp.AccountType.ASSET;
                    ...
                    if (!accountProperties[column.getName()]) {
                        accountProperties[column.getName()] = formatPropertyValue(
                            book,
                            value,
                            timezone
                        );
            ...
            accountType = isValidType(type)
                ? (type as Bkper.AccountType)
                : BkperApp.AccountType.ASSET;
        ...
        if (account) {
            batch.setExistingAccount(
                new BatchExistingAccount(book, account, accountGroupNames, accountProperties),
                `${rowIndex}`
            );
        } else {
            const newAccount = book.newAccount().setName(accountName).setType(accountType);
            batch.setNewAccount(
                new BatchNewAccount(book, newAccount, accountGroupNames, accountProperties),
                `${rowIndex}`
            );
        }
