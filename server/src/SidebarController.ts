...
function showSidebar(): void {
    if (Authorizer.isUserAuthorized()) {
        var ui = HtmlService.createTemplateFromFile('SidebarView')
            .evaluate()
...
function loadLedgers(): ClientBook[] {
    try {
        ...
        return ledgers.map(book => {
            return {
                id: book.getId(),
                name: book.getName(),
                viewer: book.getPermission() == BkperApp.Permission.VIEWER,
                selected: book.getId() == lastSelectedLedger,
            };
        });
    } catch (err) {
        if (!Authorizer.isUserAuthorized()) {
