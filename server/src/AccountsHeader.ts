class AccountsHeader {
    ...
    private columns: AccountsHeaderColumn[];
    private rowNum: number;

    private bookIdHeaderColumn: AccountsHeaderColumn;
    ...
    private parse() {
        ...
        for (let i = 0; i < headerValues.length; i++) {
            for (let j = 0; j < headerValues[i].length; j++) {
                const header = new AccountsHeaderColumn(headerValues[i][j], j);
    ...
    getBookIdHeaderColumn(): AccountsHeaderColumn {
    ...
    getColumns(): AccountsHeaderColumn[] {
        return this.columns;
    }

}

class AccountsHeaderColumn {
