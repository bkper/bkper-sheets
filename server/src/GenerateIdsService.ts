...
namespace GenerateIdsService {
    ...
    export function generate(sheet: GoogleAppsScript.Spreadsheet.Sheet): void {
        ...
        if (!header.isValid()) {
            const bkperHelpUrl =
                'https://help.bkper.com/en/articles/2569151-bkper-add-on-for-google-sheets#h_73cf0eaf6c';
        ...
        if (idColumn == 0) {
            const htmlOutput = Utilities_.getErrorHtmlOutput(
                'Could not find an ID column in the header.'
            );
            SpreadsheetApp.getUi().showModalDialog(htmlOutput, 'Error');
            return;
        }
    ...
    function fillIdColumn(
        ...
        if (transactionsRange.isBlank()) {
            const htmlOutput = Utilities_.getErrorHtmlOutput(
                'Sheet appears to contain no transactions below the header.'
            );
            SpreadsheetApp.getUi().showModalDialog(htmlOutput, 'Error');
            return;
        }

        const idColumnData = sheet
            .getRange(headerRowNum, idColumn, sheet.getLastRow(), 1)
            .getValues();
        const idColumnBackgrounds = sheet
            .getRange(headerRowNum, idColumn, sheet.getLastRow(), 1)
            .getBackgrounds();
        ...
        for (let i = 0; i < idColumnData.length; i++) {
            ...
            if (cellValue) {
                ...
                const emptyRow = sheet
                    .getRange(headerRowNum + i, 1, 1, sheet.getLastColumn())
                    .isBlank();
                if (!emptyRow) {
                    ...
                    // Background
                    cellBackground === ERROR_BACKGROUND_
                        ? idColumnNewBackgrounds.push([undefined])
                        : idColumnNewBackgrounds.push([cellBackground]);
                } else {
                    // Data
        ...
        sheet
            .getRange(headerRowNum, idColumn, idColumnNewData.length, 1)
            .setValues(idColumnNewData)
            .setBackgrounds(idColumnNewBackgrounds);
    }
}
